...
void prefixans(v64 &v) { forsn(i, 1, sz(v)) v[i] += v[i - 1]; }
void postfixans(v64 &v) { rforsn(i, sz(v) - 2, 0) v[i] += v[i + 1]; }
...
void solution()
{
    ...
    {
        ll par = q.front().fi;
        ll ans = q.front().se.fi;
        ll res = q.front().se.se;
        q.pop();
        res += ans;
        ans += v[par-1];
        res += v[par-1];
