...
void solution()
{
    ...
    forn(i, n)
    {
        ...
        else
        {
            if (v[i] == 0)
            {
                dp[i].fi = dp[i].se = max(dp[i - 1].fi, dp[i - 1].se);
            }
            else if (v[i] == 1)
            {
                dp[i].fi = max(dp[i - 1].se + 1, dp[i - 1].fi);
                dp[i].se = max(dp[i - 1].se, dp[i - 1].fi);
            }
            else if (v[i] == 2)
            {
                dp[i].se = max(dp[i - 1].fi + 1, dp[i - 1].se);
                dp[i].fi = max(dp[i - 1].se, dp[i - 1].fi);
            }
            else
            {
                dp[i].fi = dp[i - 1].se + 1;
        ...
        // cout<<v[i]<<" "<<dp[i].fi<<" "<<dp[i].se<<nl;
    }
    cout << n - max(dp[n - 1].fi, dp[n - 1].se);
}