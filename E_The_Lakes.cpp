        ...
        forn(j, b)
        {
            ...
            if (visi[{i, j}] == 0 && v[i][j] != 0)
            {
                ...
                while (!q.empty())
                {
                    ...
                    forn(ii, 4)
                    {
                        if (isValidPlace(x + dx[ii], y + dy[ii], a, b) && v[x + dx[ii]][y + dy[ii]] != 0 && visi[{x + dx[ii], y + dy[ii]}] == 0)
                        {
                            visi[{x + dx[ii], y + dy[ii]}] = 1;
                            ans += v[x + dx[ii]][y + dy[ii]];
                            q.push({x + dx[ii], y + dy[ii]});
                        }
                    }
                }
                mx = max(ans, mx);
