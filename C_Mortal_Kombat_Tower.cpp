...
ll f(v64 &v, ll a, ll i, bool turn,  vector<vector<int>> &dp)
{
    ...
    // its my turn
    {
        mx = min(f(v, a, i + 2, !turn, dp), f(v, a, i + 1, !turn, dp));
    }
    else
    // its my frd turn
    {
        mx = f(v, a, i + 1, !turn, dp) + v[i];
        if (i + 1 < a)
        {
            mx = min(mx, f(v, a, i + 2, !turn, dp) + v[i] + v[i + 1]);
        }
    }
    return dp[i][turn] = mx;
}
void solution()
{
    ...
    v64 v(a);
    forn(i, a) cin >> v[i];
    vector<vector<int>> dp(a + 10, vector<int> (10, -1));
    bool turn = 0;
    ans = f(v, a, 0, turn, dp);
    cout << ans;
}