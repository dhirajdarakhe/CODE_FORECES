...
int f(vector<int> &v, int n, vector<vector<int>> &dp, int i, vector<vector<int>> &index, int fg)
{
    if (i >= n)
        return 0;
    if (dp[i][fg] != -1)
        return dp[i][fg];
    int tk = fg + f(v, n, dp, i + 1, index, 1);
    int ntk = 1e7;
    int id = ub(all(index[v[i]]), i) - index[v[i]].begin();
    if (id != sz(index[v[i]]))
    {
        // cout<<v[i]<<" "<<v[index[v[i]][id]]<<" "<<i<<" "<<index[v[i]][id]<<nl;
        ntk = f(v, n, dp, index[v[i]][id], index, 0);
    }
    // cout << i << " " << tk << " " << ntk << " " << dp[1] << nl;
    return dp[i][fg] = min(tk, ntk);
}
void solution()
{
    // int mn = 2e18;
    ...
    v64 v(n);
    forn(i, n) cin >> v[i];
    vector<vector<int>> index(n + 10);
    for (int i = 0; i < n; ++i)
        index[v[i]].pb(i);
    vector<vector<int>> dp(n + 10, vector<int>(3, -1));
    cout << n - f(v, n, dp, 0, index, 1);
}
