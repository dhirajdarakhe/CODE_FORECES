...
void solution()
{
    ...
    vector<pair<int, int>> u(a, {1ll<<60, 1ll<<60});
    forn(i, a)
    {
        if (i == 0)
        {
        ...
        string y2 = w[i].se;
        if (y1 >= x1)
            u[i].fi = min(u[i].fi, u[i - 1].fi + 0);
        if (y1 >= x2)
            u[i].fi = min(u[i].fi, u[i - 1].se + 0);
        if (y2 >= x1)
            u[i].se = min(u[i].se, u[i - 1].fi + v[i]);
        if (y2 >= x2)
            u[i].se = min(u[i].se, u[i - 1].se + v[i]);
    }
    // cout<<(11254274876 >= INT_MAX)<<endl;
    ans = min(u[a - 1].fi, u[a - 1].se);
    if (ans != 1ll<<60)
        cout << ans;
    else
        cout << -1;
